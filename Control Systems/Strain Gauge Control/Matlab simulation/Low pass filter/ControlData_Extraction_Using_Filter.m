%% ================================================================
%  3 Op-Amp Instrumentation Amplifier — Full Circuit Analysis
% ================================================================
%  TOPOLOGY:
%    Stage 1 : Two non-inverting buffers
%              R1 = R2 = 10 kΩ  (feedback)
%              Rg = 322 Ω       (gain-set)
%    Stage 2 : Difference amplifier
%              R3 = 2 kΩ  (input resistors from stage-1 outputs)
%              R4 = 32 kΩ (feedback)
%  INPUTS    : V1 = 5 mV,  V2 = 0 mV
%  BW TARGET : 0 – 5 Hz
%  Op-Amp GBW: 1 MHz (assumed, e.g. TL071 / LM324 class)
% ================================================================

clear; clc; close all;

%% ─── 1. CIRCUIT PARAMETERS ──────────────────────────────────────
R1  = 10e3;          % Stage-1 feedback resistors (Ω)
R2  = 10e3;
Rg  = 322;           % Gain-setting resistor (Ω)
R3  = 2e3;           % Stage-2 input resistors (Ω)
R4  = 32e3;          % Stage-2 feedback resistor (Ω)

V1  = 5e-3;          % Input V1 (V)
V2  = 0;             % Input V2 (V)
Vdd = 15;            % Supply rail (±15 V assumed)

GBW = 1e6;           % Op-amp gain-bandwidth product (Hz)
k   = 1.38e-23;      % Boltzmann constant
T   = 300;           % Temperature (K)
BW_op = 5;           % Operating bandwidth (Hz)

%% ─── 2. GAIN CALCULATIONS ───────────────────────────────────────
Vdiff = V1 - V2;
Vcm   = (V1 + V2) / 2;

A1      = 1 + (2*R1)/Rg;      % Stage-1 differential gain
A2      = R4 / R3;             % Stage-2 gain
A_total = A1 * A2;             % Overall gain

BW1 = GBW / A1;               % Stage-1 –3 dB BW (Hz)
BW2 = GBW / A2;               % Stage-2 –3 dB BW (Hz)
% Cascaded –3 dB bandwidth (Butterworth approximation)
BW_sys = sqrt((sqrt(2)-1)) * min(BW1, BW2);   % rough estimate

Vout_dc = A_total * Vdiff;     % DC output voltage
