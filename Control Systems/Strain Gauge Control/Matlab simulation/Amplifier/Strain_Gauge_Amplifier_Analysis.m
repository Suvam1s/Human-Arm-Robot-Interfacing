...
%% ─── GLOBAL DARK THEME (must be set BEFORE any figure is created) ────
...
set(0,'DefaultAxesMinorGridColor',[0.3 0.3 0.3]);
...
figure('Name','Fig 1 – Bode Plot','NumberTitle','off',...
       'Position',[40 40 950 700],'Color','k');
...
yline(20*log10(A_total)-3,'w:','LineWidth',1.4,'Label','–3 dB','LabelHorizontalAlignment','left');
xline(5,   'Color',[0.8 0.4 1],'LineWidth',1.8,'Label','5 Hz (target BW)','LabelVerticalAlignment','top');
...
title('Bode Plot — 3 Op-Amp INA','FontSize',13,'FontWeight','bold','Color','w');
legend('Overall','Stage 1','Stage 2','Location','southwest','FontSize',9,...
       'TextColor','w','Color','k','EdgeColor',[0.5 0.5 0.5]);
set(ax1,'FontSize',10,'Color','k','XColor','w','YColor','w');
...
xline(5,'Color',[0.8 0.4 1],'LineWidth',1.8,'Label','5 Hz');
yline(-45, 'w:','LineWidth',1.2,'Label','–45°');
yline(-90, 'w:','LineWidth',1.2,'Label','–90°');
xlim([0.01 1e6]); grid on; box on;
xlabel('Frequency (Hz)','FontWeight','bold');
ylabel('Phase (°)','FontWeight','bold');
legend('Overall','Stage 1','Stage 2','Location','southwest','FontSize',9,...
       'TextColor','w','Color','k','EdgeColor',[0.5 0.5 0.5]);
set(ax2,'FontSize',10,'Color','k','XColor','w','YColor','w');

%% ═══════════════════════════════════════════════════════════════
%  FIGURE 2 — GAIN IN OPERATING BAND (0–5 Hz)
% ═══════════════════════════════════════════════════════════════
figure('Name','Fig 2 – Operating Band Gain','NumberTitle','off',...
       'Position',[60 60 820 420],'Color','k');
...
xlabel('Frequency (Hz)','FontWeight','bold');
title('Voltage Gain in Operating Range (0–5 Hz)','FontSize',12,'FontWeight','bold','Color','w');
...
title('Output for Multiple Input Frequencies (Vdiff = 5 mV)','FontWeight','bold','Color','w');
legend(labels,'Location','northeast','FontSize',9,...
       'TextColor','w','Color','k','EdgeColor',[0.5 0.5 0.5]);
grid on; box on;
yline(Vout_dc,'w--','LineWidth',1.2,'Label',sprintf('DC Vout=%.4fV',Vout_dc));
set(gca,'Color','k','XColor','w','YColor','w');
...
legend('V1','V2','FontSize',9,'TextColor','w','Color','k','EdgeColor',[0.5 0.5 0.5]);
grid on; box on;
xlabel('Time (s)'); ylabel('Voltage (mV)');
title('Input Signals @ 1 Hz','FontWeight','bold','Color','w');
set(gca,'Color','k','XColor','w','YColor','w');
...
title('Differential Input @ 1 Hz','FontWeight','bold','Color','w');
set(gca,'Color','k','XColor','w','YColor','w');
...
title('Output Voltage @ 1 Hz','FontWeight','bold','Color','w');
...
legend('V1 (mV)','Vout (V)','FontSize',9,'Location','east',...
       'TextColor','w','Color','k','EdgeColor',[0.5 0.5 0.5]);
title('DC Operating Point','FontWeight','bold','Color','w');                                  
grid on; box on;
set(gca,'Color','k','XColor','w','YColor','w');
...
figure('Name','Fig 4 – Transfer Characteristic','NumberTitle','off',...
       'Position',[100 100 780 540],'Color','k');
...
plot(Vdiff_sw*1e3, Vout_ideal, 'c--', 'LineWidth',1.5,'DisplayName','Ideal (no rail)'); hold on;
...
text(Vdiff*1e3 + 0.5, Vout_dc - 0.8,...
    ...
    'FontSize',10,'Color',[1 0.5 0.5],'FontWeight','bold');
...
title(sprintf('Transfer Characteristic  |  Total Gain = %.2f (%.2f dB)',...
    A_total, 20*log10(A_total)),'FontWeight','bold','FontSize',12,'Color','w');
...
figure('Name','Fig 5 – CMRR Analysis','NumberTitle','off',...
       'Position',[120 120 900 500],'Color','k');
...
text(0.2, CMRR_op-5, sprintf('%.1f dB at 0.1%% mismatch', CMRR_op),...
    'FontSize',9,'Color',[1 0.5 0.5]);

xlabel('Resistor Mismatch (%)','FontWeight','bold');
ylabel('CMRR (dB)','FontWeight','bold');
title('CMRR vs Resistor Mismatch','FontWeight','bold','Color','w');
xlim([0 5]); grid on; box on;
set(gca,'Color','k','XColor','w','YColor','w');
...
xline(5,'w--','LineWidth',1.5,'Label','5 Hz operating limit');
yline(80,'g--','LineWidth',1.2); yline(60,'r--','LineWidth',1.2);
xlabel('Frequency (Hz)','FontWeight','bold');
ylabel('CMRR (dB)','FontWeight','bold');
title('CMRR vs Frequency','FontWeight','bold','Color','w');
...
text(6, CMRR_5Hz-4, sprintf('%.1f dB @ 5 Hz', CMRR_5Hz),'FontSize',9,'Color',[1 0.5 0.5]);

%% ═══════════════════════════════════════════════════════════════
%  FIGURE 6 — NOISE ANALYSIS
% ═══════════════════════════════════════════════════════════════
figure('Name','Fig 6 – Noise Analysis','NumberTitle','off',...
       'Position',[140 140 950 600],'Color','k');
...
b.CData = bar_clr;
set(gca,'XTickLabel',resistors,'FontSize',9,'Color','k','XColor','w','YColor','w');
xtickangle(30);
xlabel('Resistor','FontWeight','bold');
ylabel('Input-Referred Noise (nV)','FontWeight','bold');
title(sprintf('Thermal Noise RTI  (BW = %d Hz, T = %d K)',BW_op,T),...
      'FontWeight','bold','Color','w');
yline(RTI_total,'w--','LineWidth',1.5,...
...
yline(RTI_total/sqrt(BW_op),'c--','LineWidth',1.4,...
    'Label',sprintf('Thermal floor = %.3f nV/√Hz', RTI_total/sqrt(BW_op)));
xlabel('Frequency (Hz)','FontWeight','bold');
ylabel('Noise Density (nV/√Hz)','FontWeight','bold');
title('Input-Referred Noise Spectral Density (0–5 Hz)','FontWeight','bold','Color','w');
...
figure('Name','Fig 7 – Gain Sensitivity','NumberTitle','off',...
       'Position',[160 160 900 450],'Color','k');
...
xline(Rg,'w--','LineWidth',1.8,...
    'Label',sprintf('Rg=322Ω  G=%.0f',A_total));
xlabel('Rg (Ω)','FontWeight','bold');
title('Gain vs Rg','FontWeight','bold','Color','w');
grid on; box on;
set(gca,'Color','k','XColor','w','YColor','w');
...
title(sprintf('Vout vs Rg  (Vdiff = %.0f mV)',Vdiff*1e3),'FontWeight','bold','Color','w');
grid on; box on;
set(gca,'Color','k','XColor','w','YColor','w');
...
figure('Name','Fig 8 – Pole-Zero & Step Response','NumberTitle','off',...
       'Position',[180 180 950 480],'Color','k');
...
plot(0, 0,'wo','MarkerSize',10,'LineWidth',2,'DisplayName','No zeros');
xline(0,'w-','LineWidth',0.8); yline(0,'w-','LineWidth',0.8);
...
title('Pole-Zero Map','FontWeight','bold','Color','w');
...
yline(0.9*Vout_dc, 'w:','LineWidth',1.2,'Label','90%');
yline(0.1*Vout_dc, 'w:','LineWidth',1.2,'Label','10%');
xlabel('Time (s)','FontWeight','bold');
ylabel('Output Voltage (V)','FontWeight','bold');
title('Approximate Step Response','FontWeight','bold','Color','w');
grid on; box on;
set(gca,'Color','k','XColor','w','YColor','w');
